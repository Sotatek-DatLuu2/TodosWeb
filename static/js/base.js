    ...
    // Hàm chung để chuyển hướng với token (cho DEBUG ONLY)
    function redirectToPageWithToken(path) {
        const token = getCookie('access_token');
        if (token) {
            window.location.href = `${path}?token=${token}`;
        } else {
            window.location.href = '/auth/login-page'; // Fallback nếu không có token
        }
    }
        ...
        todoForm.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    redirectToPageWithToken('/todos/todo-page'); // Sử dụng hàm mới
                } else {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.user_role === 'admin') {
                        redirectToPageWithToken('/admin/all-todos-page'); // Sử dụng hàm mới
                    } else {
                        redirectToPageWithToken('/todos/todo-page'); // Sử dụng hàm mới
                    }
                } else {
